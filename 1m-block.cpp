	      ...
	      struct nfq_data *nfa, void *data)
{
	...
	if(ip_hdr->ip_p == 0x06 && (ntohs(tcp_hdr->th_sport) == 80 || ntohs(tcp_hdr->th_dport) == 80)){
		...
		for (int i = 0; i < ret; i++){
			if (strncmp(payload+i, "Host:", 5) == 0){
				...
				std::string host;
				int j = 0;
				while (*(packet_host + j) == 0x0d ){
					host.append(std::to_string(*(packet_host+j)));
					j++;
				}
				if (HostsHash.find(host) != HostsHash.end()) {
					printf("Host Rejected\n");
					STOP_TIME;
					PRINT_TIME("Control Rejected Packet");
	...
	STOP_TIME;
	PRINT_TIME("Control Normal Packet");
...
int main(int argc, char **argv)
{
	...
	START_TIME;
    	while (!fp.eof()) {
    		std::getline(fp, host, '\n');
        	HostsHash.insert(host);
    	}
    	STOP_TIME;
    	PRINT_TIME("Making Hash Table");
    	fp.close();
